(()=>{var a={};a.id=6100,a.ids=[6100],a.modules={261:a=>{"use strict";a.exports=require("next/dist/shared/lib/router/utils/app-paths")},3295:a=>{"use strict";a.exports=require("next/dist/server/app-render/after-task-async-storage.external.js")},10846:a=>{"use strict";a.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},13083:(a,b,c)=>{"use strict";c.r(b),c.d(b,{handler:()=>H,patchFetch:()=>G,routeModule:()=>C,serverHooks:()=>F,workAsyncStorage:()=>D,workUnitAsyncStorage:()=>E});var d={};c.r(d),c.d(d,{DELETE:()=>A,PATCH:()=>z,POST:()=>B,dynamic:()=>y,runtime:()=>x});var e=c(50850),f=c(18523),g=c(666),h=c(124),i=c(21058),j=c(261),k=c(57396),l=c(88126),m=c(3510),n=c(59365),o=c(58067),p=c(69545),q=c(11711),r=c(70840),s=c(86439),t=c(28806),u=c(73283),v=c(96048),w=c(65936);let x="nodejs",y="force-dynamic";async function z(a,{params:b}){let{id:c}=await b,{role:d}=await a.json();if(!["super_admin","admin","finance","support"].includes(d))return u.NextResponse.json({error:"Invalid role"},{status:422});let{error:e}=await v.E.auth.admin.updateUserById(c,{user_metadata:{role:d}});return e?u.NextResponse.json({error:e.message},{status:500}):u.NextResponse.json({ok:!0})}async function A(a,{params:b}){let{id:c}=await b,{error:d}=await v.E.auth.admin.deleteUser(c);return d?u.NextResponse.json({error:d.message},{status:500}):u.NextResponse.json({ok:!0})}async function B(a,{params:b}){let c,{id:d}=await b,{data:{user:e},error:f}=await v.E.auth.admin.getUserById(d);if(f||!e?.email)return u.NextResponse.json({error:"User not found"},{status:404});let g=process.env.NEXT_PUBLIC_ADMIN_URL??"http://localhost:3001",h=!!e.email_confirmed_at,{data:i,error:j}=await v.E.auth.admin.generateLink({type:"recovery",email:e.email,options:{redirectTo:`${g}/set-password`}});if(j)return u.NextResponse.json({error:j.message},{status:500});let k=i.properties.action_link,l=e.user_metadata?.full_name??e.email.split("@")[0],m=e.user_metadata?.role??"admin";return(c=h?await (0,w.ZM)({to:e.email,subject:"Reset your AlgoLend password",html:`<!DOCTYPE html>
...
<div style="max-width:600px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,0.08)">
  <div style="background:linear-gradient(135deg,#7C3AED,#9B5CF6);padding:32px;text-align:center">
    ...
    <p style="color:rgba(255,255,255,0.75);font-size:13px;margin:6px 0 0">AlgoLend \xb7 Mint Platforms</p>
  </div>
  <div style="padding:32px">
    <p style="font-size:15px;margin:0 0 16px">Hi ${l},</p>
    <p style="color:#555;line-height:1.6;margin:0 0 24px">
      A password reset was requested for your AlgoLend admin account (<strong>${e.email}</strong>).
      Click the button below to set a new password.
    </p>
    <div style="text-align:center;margin:28px 0">
      <a href="${k}" style="background:#7C3AED;color:#fff;padding:14px 36px;border-radius:10px;text-decoration:none;font-weight:700;font-size:15px;display:inline-block">
        Reset password →
      </a>
    </div>

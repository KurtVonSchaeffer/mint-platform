...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!isPdf && !isImage) {
      return NextResponse.json({ error: 'Upload a PDF or image (JPG, PNG).' }, { status: 422 });
    }
    ...
    console.error('[public/ncr-ocr]', err);
    return NextResponse.json({ error: 'OCR failed — please enter the NCR number manually.' }, { status: 500 });
  }
}

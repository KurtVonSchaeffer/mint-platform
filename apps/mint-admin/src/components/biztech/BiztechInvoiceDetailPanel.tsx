      ...
      <div className="slideover-panel w-full max-w-lg flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between p-7" style={{ borderBottom: '1px solid var(--color-border2)' }}>
          <div>
            ...
            <h2 className="font-mono text-base font-bold tracking-tight leading-tight" style={{ color: 'var(--color-text)' }}>{invoice.reference}</h2>
            <p className="text-sm mt-1" style={{ color: 'var(--color-text3)' }}>{invoice.biztech_clients?.name ?? '—'}</p>
          </div>

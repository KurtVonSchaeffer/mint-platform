...
function CopyBtn({ value }: { value: string }) {
  ...
  return (
    <button
      onClick={copy}
      title="Copy"
      style={{ background: 'none', border: 'none', cursor: 'pointer', color: copied ? '#34d399' : '#52525b', padding: '2px 4px', borderRadius: 4, transition: 'color 0.15s' }}
    >
...
export function BiztechInvoicePage(props: Props) {
    ...
    return (
      <div style={s}>
        <div style={{ maxWidth: 480, margin: '0 auto', textAlign: 'center', paddingTop: 60 }}>
          ...
          <button onClick={() => window.location.href = `/biztech-invoice/${props.invoiceId}`} style={{ background: 'linear-gradient(135deg,#31005E,#5C3BCF)', color: '#fff', border: 'none', borderRadius: 10, padding: '12px 28px', fontWeight: 700, cursor: 'pointer', fontSize: 14 }}>
            Try again
          </button>
        </div>
      </div>
    ...
    <div style={s}>
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(49,0,94,0.5), rgba(92,59,207,0.25))', border: '1px solid rgba(92,59,207,0.3)', borderRadius: 14, padding: '20px 24px', marginBottom: 24 }}>
          <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#DDC357', margin: '0 0 14px' }}>Invoice</p>
        ...
        {props.payfastFields && (
          <div style={{ background: 'rgba(92,59,207,0.08)', border: '1px solid rgba(92,59,207,0.25)', borderRadius: 14, padding: '20px 24px', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              <CreditCard size={16} color="#8b6ce8" />
            ...
            <button
              ...
              style={{ width: '100%', background: 'linear-gradient(135deg,#31005E,#5C3BCF)', color: '#fff', border: 'none', borderRadius: 10, padding: '13px', fontWeight: 700, fontSize: 14, cursor: 'pointer', boxShadow: '0 6px 20px -4px rgba(92,59,207,0.45)' }}
            >
        ...
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '20px 24px' }}>
          ...
          <p style={{ fontSize: 11, color: '#52525b', margin: '12px 0 0', lineHeight: 1.6 }}>
            ...
            {' '}<a href="mailto:accounts@mymint.co.za" style={{ color: '#8b6ce8' }}>accounts@mymint.co.za</a>{' '}

...
function renderActiveLoans() {
        ...
        <div class="modern-list-item" onclick="openLoansModule()">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                ...
                <div style="text-align:right;">
                    ...
                    <span style="display:inline-block; margin-top:6px; padding:4px 12px; background:rgb(var(--color-primary-rgb) / 0.1); color:var(--color-primary); border-radius:50px; font-size:10px; font-weight:700; text-transform:uppercase;">Active</span>
                </div>
            </div>
        </div>
    `).join('');
    ...
    if (activeLoans.length > 3) {
        html += `
            <button class="text-btn" onclick="openLoansModule()" style="width: 100%; text-align: center; margin-top: 16px; padding: 12px; font-weight: 600; color: var(--color-primary); background: rgb(var(--color-primary-rgb) / 0.05); border-radius: var(--radius-md); transition: 0.2s; cursor: pointer;">
...
function renderBankAccounts() {
    ...
    let html = visibleAccounts.map(account => `
        <div class="modern-list-item" style="display:flex; flex-direction:row; justify-content:space-between; align-items:center;">
            ...
            <div style="display: flex; align-items: center; gap: 16px;">
                ${account.isPrimary ? '<span class="status-badge completed" style="background:rgb(var(--color-primary-rgb) / 0.1); color:var(--color-primary);">Primary</span>' : ''}
    ...
    if (bankAccounts.length > 3) {
        html += `
            <button class="text-btn" onclick="openBankAccountsModule()" style="width: 100%; text-align: center; margin-top: 16px; padding: 12px; font-weight: 600; color: var(--color-primary); background: rgb(var(--color-primary-rgb) / 0.05); border-radius: var(--radius-md); transition: 0.2s; cursor: pointer;">
    ...
    const listHtml = active.length === 0 ? '<div class="empty-state">No active loans found.</div>' : active.map(loan => `
            <div class="modern-list-item" style="border: 1px solid #eee; background: var(--color-white); box-shadow: var(--shadow-soft);">
                <div class="modern-item-header">
                    ...
                    <span class="status-badge" style="background:rgb(var(--color-primary-rgb) / 0.1); color:var(--color-primary);">Active</span>
                </div>
...
window.downloadStatement = async function() {
    ...
    try {
        ...
        doc.autoTable({
            ...
            theme: 'grid',
            headStyles: { 
                fillColor: [var(--color-primary-rgb)], 

...
const getThemePalette = () => {
    ...
    const primaryRgb = read('--color-primary-rgb', '124 58 237');
    return {
        primary: read('--color-primary', 'var(--color-primary, #7C3AED)'),
        surfaceCard: read('--color-surface-card', '#FFFFFF'),
        primaryAlpha: (alpha) => { const p = primaryRgb.trim().split(/\s+/); return `rgba(${p.join(', ')}, ${alpha})`; }
    };
};
                ...
                return `
                    <div class="loan-card-hifi">
                        <div style="display: flex; justify-content: space-between; margin-bottom: 12px; align-items: center;">
                            ...
                            <span style="background: rgb(var(--color-primary-rgb) / 0.1); color: var(--color-primary); padding: 6px 12px; border-radius: 100px; font-size: 11px; font-weight: 700; text-transform: uppercase;">${loan.status}</span>
                        </div>
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        titleColor: '#fff',
                        bodyColor: '#ffead6',
                        borderColor: 'rgb(var(--color-primary-rgb) / 0.3)',
                        borderWidth: 1,
                        padding: 14,
                        displayColors: false,
                        cornerRadius: 12,
    ...
    const listHtml = active.length === 0 ? '<div style="text-align:center; padding:40px; color:var(--text-muted);">No active loans found.</div></div>' : active.map(loan => `
            <div class="modern-list-item">
                <div class="modern-item-header">
                    ...
                    <span class="status-badge" style="background:rgb(var(--color-primary-rgb) / 0.1); color:var(--color-primary); padding:6px 12px; border-radius:100px; font-size:11px; font-weight:700;">${loan.status}</span>
                </div>
            ...
            : dashboardData.applications.map(app => `
                <div class="modern-list-item" style="padding: 20px; margin-bottom: 12px; border: none;">
                    ...
                    <div style="display:flex; justify-content:space-between; font-size:13px; color:var(--text-muted); font-weight:600;">
                        <span>${app.date}</span>
                        <span style="color:var(--color-primary); background: rgb(var(--color-primary-rgb) / 0.1); padding: 4px 10px; border-radius: 10px;">${app.status}</span>
                    </div>
                </div>`).join('');
    }
};

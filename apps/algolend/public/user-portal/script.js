...
// Use PNG logo if available, fallback to SVG
const DEFAULT_BRAND_LOGO = '/shared/algolend-logo.png';
const DEFAULT_BRAND_LOGO_FALLBACK = '/shared/algolend-logo.svg';
const FALLBACK_SYSTEM_THEME = {
  primary_color: 'var(--color-primary, #7C3AED)',
...
function applyBrandColors(theme) {
  ...
  const primary   = (theme?.primary_color   || 'var(--color-primary, #7C3AED)').trim();
  const secondary = (theme?.secondary_color || '#1A1F36').trim();
  ...
  root.style.setProperty('--ds-orange-rgb', primaryRgb || 'var(--color-primary-rgb)');
}
...
function populateUserDropdown(profile) {
  ...
  accountBtns.forEach(btn => {
    ...
    circle.style.cssText = `
      width: 32px; height: 32px; background: var(--color-primary, var(--color-primary, #7C3AED)); 

...
function renderAppShell(profile, role, theme = null) {
  ...
  appShell.innerHTML = `
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 flex flex-col w-[280px] bg-surface-container-lowest border-r border-outline-variant/20 transition-transform duration-300 ease-in-out md:translate-x-0 -translate-x-full">
      ...
      <!-- Sidebar top padding (logo moved to top bar) -->
      <div class="h-16"></div>
    ...
    <div class="flex flex-col flex-1 md:pl-[280px] min-h-screen bg-surface relative">
      ...
      <header class="sticky top-0 z-30 h-16 flex items-center justify-between px-8 bg-surface-container-lowest/80 backdrop-blur-xl border-b border-outline-variant/15">
        <div class="flex items-center gap-4">
          ...
          <!-- Brand in top bar — visible on all screen sizes -->
          <div class="flex items-center gap-2.5">
            ${customLogo
              ? `<img src="${logoSrc}" alt="${logoAlt}" class="h-8 w-auto object-contain max-w-[140px]">`
              : `<div class="flex items-center gap-2">
                   <div class="w-8 h-8 rounded-lg flex items-center justify-center text-white font-bold text-sm flex-shrink-0" style="background: linear-gradient(135deg, var(--color-primary), var(--color-secondary));">
                     ${(companyName || 'A').charAt(0).toUpperCase()}
                   </div>
                   <span class="font-semibold text-sm tracking-tight" style="color: var(--color-primary);">${companyName || 'Admin'}</span>
                 </div>`
            }
          </div>
          <!-- Divider -->
          <div class="h-5 w-px bg-outline-variant/30 hidden md:block"></div>
          <h1 id="page-title" class="hidden md:block text-sm font-medium text-on-surface-variant"></h1>
        </div>

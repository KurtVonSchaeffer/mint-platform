      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-3 flex-wrap">
          <select id="branch-select" onchange="window.clSwitchBranch(this.value)"
            class="text-sm border border-gray-200 rounded-xl px-3 py-2 bg-white font-semibold focus:ring-[var(--color-primary)] focus:outline-none">
          ...
          <button onclick="window.clOpenJournal()"
            class="flex items-center gap-2 bg-[var(--color-primary)] hover:bg-[var(--color-primary)] text-white text-sm font-bold px-4 py-2.5 rounded-xl transition-colors shadow-sm">
          ...
          <form id="cl-form" onsubmit="window.clSaveEntry(event)" class="space-y-4">
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input name="entry_date" type="date" value="${activeDate}" required
                  class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-[var(--color-primary)] focus:border-transparent outline-none">
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1">Type</label>
                <select name="entry_type" required class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-[var(--color-primary)] outline-none bg-white">
              ...
              <div>
                ...
                <select name="category" class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-[var(--color-primary)] outline-none bg-white">
              ...
              <div>
                ...
                <select name="branch_id" class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-[var(--color-primary)] outline-none bg-white">
            ...
            <div class="flex gap-3 pt-2">
              ...
              <button type="submit" id="cl-save-btn"
                class="flex-1 bg-[var(--color-primary)] hover:bg-[var(--color-primary)] text-white font-semibold py-2.5 rounded-xl text-sm transition-colors">Save Entry</button>
            </div>
          </form>
        </div>
      </div>
...
function renderTable() {
    ...
    if (!filtered.length) {
        tbody.innerHTML = `<tr><td colspan="9" class="p-10 text-center text-sm text-gray-400">No entries for this date. <button onclick="window.clOpenJournal()" class="text-[var(--color-primary)] font-semibold">Add the first entry.</button></td></tr>`;
        return;
    }

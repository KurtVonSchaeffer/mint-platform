...
function showToast(message, type = 'success') {
    ...
    if (type === 'error') {
        ...
        colors = 'bg-[var(--color-primary)] text-white';
                    ...
                    <div id="credit-form-content" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="flex items-start gap-3 mt-4 p-3 rounded-md bg-[var(--color-primary)/10] border border-[var(--color-primary)]">
...
async function renderUserSelection(container) {
    ...
    container.innerHTML = `
        <div class="max-w-2xl mx-auto bg-white p-8 rounded-lg border border-gray-200 shadow-sm mt-4">
            <div class="flex border-b border-gray-200 mb-6">
                <button id="tab-search" class="flex-1 py-2 text-sm font-medium text-[var(--color-primary)] border-b-2 border-[var(--color-primary)]">
                    <i class="fa-solid fa-magnifying-glass mr-2"></i>Search Existing
                </button>
                ${canCreateWalkIn ? `
                <button id="tab-create" class="flex-1 py-2 text-sm font-medium text-gray-500 hover:text-[var(--color-primary)] transition-colors">
            ...
            <div id="view-search">
                ...
                <div class="relative mb-6">
                    <input type="text" id="user-search" class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-[var(--color-primary)] focus:border-[var(--color-primary)] transition-all shadow-sm" placeholder="Start typing name or ID...">
                    <i class="fa-solid fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                    <div id="search-spinner" class="hidden absolute right-3 top-1/2 -translate-y-1/2 text-[var(--color-primary)]">
            ...
            <div id="view-create" class="hidden animate-fade-in">
                <div class="bg-[var(--color-primary)/10] border-l-4 border-[var(--color-primary)] p-4 mb-6">
                    <div class="flex">
                        <div class="flex-shrink-0"><i class="fa-solid fa-store text-[var(--color-primary)]"></i></div>
                        <div class="ml-3">
                            <p class="text-sm text-[var(--color-primary)]">You are registering a <strong>Walk-in Client</strong>. Physical branch selection is required.</p>
                        </div>
                    </div>
                </div>
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" id="new-fullname" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-[var(--color-primary)]" placeholder="e.g. John Doe">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-700 uppercase mb-1">ID Number <span class="text-red-500">*</span></label>
                        <input type="text" id="new-idnumber" maxlength="13" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-[var(--color-primary)]" placeholder="13-digit SA ID">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-gray-700 uppercase mb-1">Phone</label>
                            <input type="tel" id="new-phone" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-[var(--color-primary)]" placeholder="082...">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 uppercase mb-1">Email (Optional)</label>
                            <input type="email" id="new-email" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-[var(--color-primary)]" placeholder="Leave empty if none">
                        </div>
                    </div>
                    ...
                    <div class="p-4 bg-gray-50 border border-gray-200 rounded-lg">
                        ...
                        <select id="new-branch-id" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-[var(--color-primary)] bg-white">
            ...
            <div id="selected-user-card" class="${inBranchState.targetUser ? '' : 'hidden'} mt-6 space-y-4 animate-fade-in">
                <div class="bg-[var(--color-primary)/10] border border-[var(--color-primary)] rounded-xl p-4 flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 rounded-full bg-[var(--color-primary)] flex items-center justify-center text-[var(--color-primary)] font-bold text-xl shadow-sm">
                ...
                <div id="action-new-loan" class="bg-white border-2 border-[var(--color-primary)] border-dashed rounded-2xl p-6 shadow-md hover:bg-[var(--color-primary)/10] cursor-pointer transition-all group">
                    <div class="flex justify-between items-center">
                        <div class="flex items-center gap-4">
                            <div class="w-10 h-10 bg-[var(--color-primary)] text-white rounded-full flex items-center justify-center shadow-lg"><i class="fa-solid fa-plus"></i></div>
                            <div>
                        ...
                        <i class="fa-solid fa-chevron-right text-[var(--color-primary)]"></i>
                    </div>
                </div>
    ...
    if (canCreateWalkIn && tabCreate) {
        tabSearch.onclick = () => { 
            ...
            tabSearch.className = "flex-1 py-2 text-sm font-medium text-[var(--color-primary)] border-b-2 border-[var(--color-primary)]";
            tabCreate.className = "flex-1 py-2 text-sm font-medium text-gray-500 hover:text-[var(--color-primary)] transition-colors";
        };
        tabCreate.onclick = () => { 
            viewSearch.classList.add('hidden'); viewCreate.classList.remove('hidden'); 
            tabCreate.className = "flex-1 py-2 text-sm font-medium text-[var(--color-primary)] border-b-2 border-[var(--color-primary)]";
            tabSearch.className = "flex-1 py-2 text-sm font-medium text-gray-500 hover:text-[var(--color-primary)] transition-colors";
        };
    }
    ...
    if(input) {
        ...
        input.oninput = (e) => {
            ...
            timer = setTimeout(async () => {
                ...
                if(matches?.length > 0) {
                    results.innerHTML = matches.map(u => `<div class="p-3 hover:bg-[var(--color-primary)/10] cursor-pointer border-b last:border-0 user-option" data-id="${u.id}"><div class="font-bold text-gray-800">${u.full_name}</div><div class="text-xs text-gray-500 font-mono">ID: ${u.identity_number || 'N/A'}</div></div>`).join('');
    ...
    if (isValid) {
        ...
        contentHtml = `
            <div class="text-center py-10">
                <div class="w-20 h-20 rounded-full bg-[var(--color-primary)/15] text-[var(--color-primary)] flex items-center justify-center text-3xl mx-auto mb-4">
    ...
    container.innerHTML = `
        <div class="max-w-4xl mx-auto bg-white p-8 rounded-2xl border border-gray-200 shadow-xl mt-4 animate-fade-in">
            ...
            <div class="space-y-8">
                ...
                <div class="p-5 bg-[var(--color-primary)/10] border border-[var(--color-primary)] rounded-xl space-y-4">
                    <label class="flex items-start gap-3 cursor-pointer">
                        <input type="checkbox" id="decl_terms" class="mt-1 w-5 h-5 text-[var(--color-primary)] border-[var(--color-primary)] rounded focus:ring-[var(--color-primary)]">
                    ...
                    <label class="flex items-start gap-3 cursor-pointer">
                        <input type="checkbox" id="decl_truth" class="mt-1 w-5 h-5 text-[var(--color-primary)] border-[var(--color-primary)] rounded focus:ring-[var(--color-primary)]">
                        <span class="text-sm text-gray-700 leading-tight">
                ...
                <div>
                    ...
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-lg font-bold text-[var(--color-primary)]">${(calc.totalRate * 100).toFixed(0)}%</span>
                        </div>
                        <div class="grid grid-cols-2 gap-2 pl-4 py-2 bg-gray-900/50 rounded border-l-2 border-[var(--color-primary)]/50">
                            <div>
            ...
            <div class="md:col-span-3 space-y-6">
                <div class="bg-white p-8 rounded-2xl border border-gray-200 shadow-xl">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button id="toggle-new-bank" class="text-xs font-bold text-brand-accent bg-[var(--color-primary)/10] px-3 py-1.5 rounded-lg border border-[var(--color-primary)]">+ ADD NEW</button>
                    </div>
                ...
                <div class="p-6 bg-[var(--color-primary)/10] border border-[var(--color-primary)] rounded-2xl">
                    <label class="flex items-start gap-3 cursor-pointer">
                        <input type="checkbox" id="admin-consent" class="mt-1 w-5 h-5 text-[var(--color-primary)] rounded border-[var(--color-primary)]">
                ...
                <div class="bg-gray-800 text-white rounded-2xl shadow-xl overflow-hidden sticky top-4 border border-gray-700">
                    ...
                    <div class="p-8">
                        ...
                        <div class="space-y-4 text-sm">
                            ...
                            <div class="flex justify-between"><span class="text-gray-400">First Debit Date</span><span class="font-bold text-[var(--color-primary)]">${firstDebitDate}</span></div>
                        </div>
                    </div>
                    <div class="p-6 bg-gray-900/80 border-t border-gray-700">
                        <button id="wizard-next-btn" class="w-full bg-brand-accent hover:bg-[var(--color-primary)] text-white py-4 rounded-xl font-black text-lg shadow-2xl transition-all disabled:opacity-50">SUBMIT APPLICATION</button>
...
const renderSearchSuggestions = (apps) => {
    ...
    searchSuggestions.innerHTML = apps.map(app => `
        <a href="/admin/application-detail?id=${app.id}" class="block p-3 hover:bg-[var(--color-primary)/10] cursor-pointer border-b border-gray-200 last:border-b-0">

      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-3 flex-wrap">
          ...
          <input type="text" id="lb-search" placeholder="Search client, reference..."
            ...
            class="text-sm border border-gray-200 rounded-xl px-3 py-2 bg-white focus:ring-[var(--color-primary)] focus:outline-none w-56">
...
function renderSummary() {
    ...
    el.innerHTML = [
        { label: 'Total Loans',      value: total,                     color: 'var(--color-primary, #7C3AED)', bg: '#fff3ea', icon: 'receipt_long' },
    ...
    tbody.innerHTML = filteredLoans.map(l => {
        ...
        const daysMaturity = l.days_to_maturity !== null
            ? (l.days_to_maturity < 0
                ...
                : `<span class="${l.days_to_maturity < 30 ? 'font-bold text-[var(--color-primary)]' : 'text-gray-600'}">${l.days_to_maturity}d</span>`)
            : '<span class="text-gray-300">—</span>';

        return `
        <tr class="hover:bg-gray-50 transition-colors ${isDefault ? 'bg-red-50/30' : isArrears ? 'bg-yellow-50/30' : ''}">
          <td class="px-4 py-3"><span class="font-mono text-xs font-bold text-[var(--color-primary)]">${l.reference}</span></td>
          <td class="px-4 py-3">
          ...
          <td class="px-4 py-3 text-center">
            <a href="/admin/application-detail?id=${l.id}" class="text-gray-400 hover:text-[var(--color-primary)] transition-colors p-1.5 rounded-lg hover:bg-[var(--color-primary)/10] inline-block">

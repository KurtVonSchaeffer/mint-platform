    ...
    <div class="lg:col-span-8 flex flex-col gap-6">
       ...
       <div id="tab-contents" class="relative min-h-[400px]">
          ...
          <div id="personal-tab" class="tab-pane glass-card rounded-2xl p-8">
                ...
                <div class="flex-grow grid grid-cols-1 gap-y-5">
                   ...
                   <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-2">
                      <span class="text-sm font-medium text-outline flex items-center gap-1">
                        <span class="material-symbols-outlined text-[14px] text-[var(--color-primary)]">people</span>
                        Next of Kin
                      </span>
                      <div class="sm:col-span-2">
                         <div id="detail-nok" class="w-full p-3 bg-[var(--color-primary)/10] border border-[var(--color-primary)] rounded-xl text-on-surface text-sm font-medium"></div>
                      </div>
                   </div>
                </div>
             </div>
             ...
             <!-- Client Credit Cap -->
             <div class="mt-4 p-5 rounded-2xl border border-[var(--color-primary)] bg-[var(--color-primary)/10]/30">
               ...
               <div class="flex gap-3 items-end">
                 <div class="flex-1">
                   ...
                   <input id="credit-cap-input" type="number" min="0" step="100" placeholder="e.g. 5000"
                     class="w-full mt-1 border border-[var(--color-primary)] rounded-xl px-3 py-2 text-sm focus:ring-2 focus:outline-none bg-white"
                     style="--tw-ring-color:var(--color-primary)">
                 </div>
                 <div class="flex-1">
          ...
          <div id="loan-tab" class="tab-pane hidden glass-card rounded-2xl p-8">
             ...
             <div class="space-y-6 mb-10">
                <div class="grid grid-cols-1 sm:grid-cols-3 items-center border-b border-outline-variant/10 pb-4">
                   ...
                   <div class="sm:col-span-2 font-mono text-sm font-bold text-on-surface bg-[var(--color-primary)/10] p-2 rounded-xl inline-block border border-[var(--color-primary)]" id="detail-app-id" style="color:var(--color-primary)"></div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-3 items-center border-b border-outline-variant/10 pb-4">
...
window.viewTruidReport = () => {
    ...
    const row = (label, val, highlight = false) => val
        ? `<tr><td style="color:#888;font-size:12px;padding:5px 10px;width:35%">${label}</td><td style="font-weight:${highlight?'700':'500'};font-size:13px;padding:5px 10px;color:${highlight?'var(--color-primary, #7C3AED)':'#1a1a1a'}">${val}</td></tr>`
        : '';
...
const renderDocuments = (docs, truidInfo, kycInfo) => {
  ...
  docTypes.forEach(type => {
      ...
      const div = document.createElement('div');
      div.className = 'flex items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:border-[var(--color-primary)] transition-all group';
            ...
            div.innerHTML = `
                <div class="flex justify-between items-center">
                    ...
                    <div class="text-right">
                        ...
                        <span class="text-[10px] uppercase font-bold text-[var(--color-primary)]">${item.status}</span>
                    </div>
                </div>
            `;
...
const renderSidePanel = (app) => {
  ...
  breakdown.innerHTML = `
    ...
    <div class="mt-4">
      ...
      <div class="p-3 bg-[var(--color-primary)/10] border border-[var(--color-primary)] rounded-xl transition-all">
        <div class="flex items-center justify-between">
          <span class="text-xs text-[var(--color-primary)] font-medium">First Repayment:</span>
  ...
  if (actionsContainer) {
      ...
      if (['BUREAU_OK', 'BANK_LINKING', 'STARTED', 'AFFORD_REFER', 'BUREAU_REFER'].includes(status)) {
          const referMessage = (status === 'AFFORD_REFER' || status === 'BUREAU_REFER') 
            ? `<div class="p-3 bg-[var(--color-primary)/10] border border-[var(--color-primary)] rounded-lg mb-3 text-xs text-[var(--color-primary)] font-bold"><i class="fa-solid fa-circle-exclamation mr-1"></i> Currently Under Manual Review</div>` 
            : '';
          ...
          actionsContainer.innerHTML = `
            ...
            ${!status.includes('REFER') ? `<button onclick="updateStatus('AFFORD_REFER')" class="w-full py-3 bg-white border border-[var(--color-primary)] text-[var(--color-primary)] text-sm font-bold rounded-xl mb-2"><i class="fa-solid fa-magnifying-glass mr-2"></i> Refer</button>` : ''}
          ...
          actionsContainer.innerHTML = `
            ...
            <button onclick="window.open('/api/letters-of-demand/${app.id}', '_blank')"
              class="w-full py-3 bg-[var(--color-primary)] hover:bg-[var(--color-primary)] text-white text-sm font-bold rounded-xl flex items-center justify-center gap-2">

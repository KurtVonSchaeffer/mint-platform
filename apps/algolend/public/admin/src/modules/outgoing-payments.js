            ...
            <div class="flex items-center gap-3 w-full lg:w-auto">
                <div class="relative flex-1 lg:w-64">
                    <input type="text" id="payout-search-input" placeholder="Search ID or Name..." 
                           class="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-[var(--color-primary)] text-sm focus:bg-white transition-colors">
  ...
  if (!document.getElementById('payout-editor-modal')) {
    ...
    modal.innerHTML = `
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6" onclick="event.stopPropagation()">
        ...
        <!-- Third party fields -->
        <div id="pe-third-party-fields" class="hidden space-y-3 mb-4 p-4 bg-[var(--color-primary)/10] border border-[var(--color-primary)] rounded-xl">
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input id="pe-third-party-name" type="text" placeholder="Full name"
                class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[var(--color-primary)] outline-none">
            </div>
            <div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input id="pe-third-party-account" type="text" placeholder="Account number"
                class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[var(--color-primary)] outline-none font-mono">
            </div>
            <div>
...
window.selectPayoutMethod = async function(method) {
    ...
    document.querySelectorAll('.pe-method-btn').forEach(btn => {
        ...
        btn.classList.toggle('bg-[var(--color-primary)/10]', active);
...
function renderPayoutTable(payouts) {
    ...
    tb.innerHTML = payouts.map(p => {
        ...
        const methodBadge = {
            ...
            third_party:   { label: 'Third Party',   bg: 'bg-[var(--color-primary)/10] text-[var(--color-primary)] border-[var(--color-primary)]' },
        ...
        <tr class="hover:bg-gray-50 transition-colors group border-b border-gray-50 last:border-0">
            <td class="px-6 py-4">
                <input type="checkbox" class="payout-checkbox rounded border-gray-300 text-[var(--color-primary)] focus:ring-[var(--color-primary)] cursor-pointer" data-id="${p.id}" ${isSelected ? 'checked' : ''}>
            </td>
            ...
            <td class="px-6 py-4">
                ...
                ${isThirdParty ? `<div class="text-[10px] text-[var(--color-primary)] font-semibold">→ ${p.third_party_bank || ''} ${p.third_party_account || ''}</div>` : ''}
            ...
            <td class="px-6 py-4">
                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide border ${activeTab === 'pending' ? 'bg-[var(--color-primary)/10] text-[var(--color-primary)] border-[var(--color-primary)]' : 'bg-green-50 text-green-600 border-green-100'}">
                    ${activeTab === 'pending' ? 'Pending' : 'Paid'}
                </span>
            </td>
            <td class="px-6 py-4 text-right flex items-center justify-end gap-2">
                <button onclick="window.openPayoutEditor('${p.id}')" class="text-gray-400 hover:text-[var(--color-primary)] transition-colors p-2 rounded-full hover:bg-[var(--color-primary)/10]" title="Edit payout method">
                    <i class="fa-solid fa-pen text-xs"></i>
                </button>
                <a href="/admin/application-detail?id=${p.application_id}" class="text-gray-400 hover:text-[var(--color-primary)] transition-colors p-2 rounded-full hover:bg-[var(--color-primary)/10] inline-block">
    ...
    tableWrapper.innerHTML = `
      <div class="p-6 space-y-4">
        <div class="flex items-center justify-between mb-2">
          ...
          <button onclick="window.exportComparisonCSV()" class="text-xs font-bold text-[var(--color-primary)] border border-[var(--color-primary)] bg-[var(--color-primary)/10] hover:bg-[var(--color-primary)/15] px-3 py-1.5 rounded-lg flex items-center gap-1.5">
        ...
        <table class="w-full text-sm border-collapse">
          ...
          <tbody class="divide-y divide-gray-100">
            ${months.map((key, i) => {
                ...
                const isUp  = change > 0;
                return `
                <tr class="hover:bg-[var(--color-primary)/10]/30 transition-colors">

  ...
  <div class="fixed top-0 left-0 right-0 z-30 bg-white shadow-md md:left-64">
    <div class="flex items-center justify-between h-16 px-6">
      ...
      <div class="flex-1"></div> <div class="flex items-center space-x-3">
        ...
        <img src="${avatarUrl}" alt="User Avatar" class="w-9 h-9 rounded-full object-cover border-2 border-[var(--color-primary, #7C3AED)]" />
      </div>
    </div>
  </div>
  `;
...
function highlightActiveLink() {
  ...
  navLinks.forEach(link => {
    ...
    if (linkPath === currentPage) {
      link.classList.add('bg-[var(--color-primary, #7C3AED)]', 'text-white');
